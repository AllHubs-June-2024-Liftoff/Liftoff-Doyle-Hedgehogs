...
| **Column Name**  | **Description**                                          |
|-------------------|--------------------------------------------------------|
| `id`             | Unique book identifier / abstracted book (Google Books API ID).          |
...
| `tags`		   | JSON data type, basically an array of hashtags
---
...
| **Column Name**      | **Description**                                                                                                                                               |
|-----------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `id`                 | Auto-generated unique identifier for each bookshelf_volume.			 |
...
| `unique_book`        | A **stored computed value** (`VARCHAR(255)`) combining `bookshelf_id` and `volume_id` with an underscore (`_`). Used for quick reference to the unique book. |
| `ratings`            | A float value between 0.0 and 5.0 (only) that is the individual users rating for the book.                                                                     |
### **Primary Key**
Moved the primary key to an autogenerated id stored at `bookshelf_volume.id`

### **Unique Individual book**
The unique_book of `bookshelf_volume` table is a composite key consisting of `bookshelf_id` concatted with `_`and `volume_id`. This ensures that a single book can be uniquely linked to a specific bookshelf. Updated so it is no longer the primary key is also stored in a column for easily searching the unique book in a column - `unique_book`.
